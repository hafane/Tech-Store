...
class AuthService {
		...
		return prisma.$transaction(async prisma => {
			...
			if (usernameExist || emailExist) {
				throw SetError.BadRequestException(
					`Пользователь с почтой ${dto.email} или с никнеймом ${dto.username} уже существует`
				)
			}
			...
			await MailService.sendRegistrationMail(
				user.email,
				user.username,
				user.activation
			)
			...
			return {
				refreshToken: tokensAndUser.refreshToken,
				accessToken: tokensAndUser.accessToken,
	...
	async login(dto: LoginDTO): Promise<IPromise> {
		...
		return {
			refreshToken: tokenAndUser.refreshToken,
			accessToken: tokenAndUser.accessToken,
